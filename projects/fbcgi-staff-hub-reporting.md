# FBCGI Staff Hub — Reporting & Dashboards

**Type:** Project record / handoff
**Last updated:** 2026-08-17
**Status:** Active — Phase 1 (manual monthly snapshots) shipped; several Phase 2 live-feed builds scoped but not started
**Owner:** G. Thomas
...
- **Latest commit at handoff:** `7574a3f1ce25bb5f34f3f45a89e8bc9528776f19`
- **Stack:** Vite + React + shadcn/ui + Tailwind + TypeScript, React Router, Recharts.
- **How to make changes:** Lovable-managed repo. Do NOT hand-edit files outside Lovable's tooling — use the Lovable MCP `send_message` tool (or the Lovable editor UI) so the agent's build pipeline stays in sync. `read_file` / `list_files` are fine for inspection.
...
## 2. What was built (2026-08-17 session)
...
- **Static data, Phase 1** — no live feed. Refresh = re-run by hand each month from the new Monthly Fleet Report export (same model as the Facilities Inspections dashboard's FMX numbers).
...
- **Data source — locked in:** Google Sheet
  ...
  Data present Jan–Jul 2026; Aug–Dec blank (shown as "pending"). YTD totals through July: 378 MarComm requests, 2.98% avg click rate, 49.15% avg open rate, 1,090,323 website visits, 23,638 podcast downloads, 2,271 Apple / 440 Android first-time downloads (Android tracked from April on).
...
- Also fixed the pre-existing digest dashboard: the "Send-by-send detail" panel on `src/pages/cbo/MarketingDigestDashboard.tsx` now loads **collapsed by default** (`defaultOpen={false}`).

### E. CBO division passcode
- In `src/components/PasscodeGate.tsx`: `SCOPE_PASSCODES.cbo` changed from `"Jones DR"` to **`jonesje`**.
- New behavior: the `cbo` scope (`/divisions/cbo/dashboards`) is now in a `NON_PERSISTENT_SCOPES` set — its unlock is **never written to sessionStorage**. Every other scope keeps the old behavior (stays unlocked for the rest of the browser session once entered). For `cbo` specifically, navigating away and back — or reloading — requires the passcode again, every time.
...
## 3. ActiveCampaign / Asana auto-pull question (answered this session)

Marketing team asked whether stats could be pulled automatically instead of typed by hand. Checked Lovable's connector catalog (`list_connectors`):

- **Asana:** available as a standard Lovable connector (not yet connected/authorized). Could feed task/project data, but Asana doesn't hold email/subscriber/social/web analytics — only useful if some metrics (e.g. MarComm request volume) are tracked as Asana tasks. Needs a decision on whether MarComm requests actually live in Asana.
- **ActiveCampaign:** **not** in Lovable's connector catalog. No one-click integration. Auto-pull would require a custom build: ActiveCampaign API token + a scheduled Supabase edge function to pull and cache metrics (same architecture as the Facilities Inspections Smartsheet feed / FMX closed-loop data). Real, scoped Phase 2 project — needs an API token from whoever admins ActiveCampaign, and a decision on which fields actually come from ActiveCampaign vs. elsewhere (social/app-store numbers don't live in ActiveCampaign).

**Bottom line:** nothing auto-pulls yet. Everything shipped (Fleet, HR, Marketing Metrics) is a manually-refreshed monthly snapshot, same pattern as the original Facilities Inspections build. Wiring any of them to a live source is a separate, explicitly-scoped follow-up.

---

## 4. Current `DEPARTMENTS` picture (post-session)
...
CBO "no dashboard yet" list is now: Events, Information Technology, Safety & Security (Human Resources removed).
...
Give the new AI:
1. This document.
2. Lovable project ID `55f65b31-3b4a-4c70-b01d-3d8d0b95f287` and workspace ID `workspace_01krh7ye84e42az2tmh7hnca6v` (needs the Lovable MCP connector authorized).
3. Confirm whether any of Fleet / HR / Marketing Metrics needs a data refresh before further changes — all frozen at July 2026.
